...
func certIsRevokedCRL(cert *x509.Certificate, url string) (revoked, ok bool) {
	...
	if shouldFetchCRL {
		...
		if issuer != nil {
			_, isGM := issuer.PublicKey.(*primitive.Sm2PublicKey) ;
			if isGM {
				err = gmx509.GetX509SM2().CheckCRLSignature(issuer,crl)
...
func fetchRemote(url string) (*x509.Certificate, error) {
	...
	if err !=nil{
		cert, err = gmx509.GetX509SM2().ParseCertificate(in)
	}
	return cert ,err
}
...
func ParseResponseForGMCert(bytes []byte, cert, issuer *x509.Certificate) (*ocsp.Response, error) {
	...
	if len(basicResp.Certificates) > 0 {
		ret.Certificate, err = gmx509.GetX509SM2().ParseCertificate(basicResp.Certificates[0].FullBytes)
		if err != nil {
			return nil, err
		}
...
//		if err := ret.CheckSignatureFrom(ret.Certificate); err != nil {
		...
		if issuer != nil {
			if err := gmx509.GetX509SM2().CheckCertSignature(issuer, ret.Certificate.SignatureAlgorithm, ret.Certificate.RawTBSCertificate, ret.Certificate.Signature); err != nil {
...
func CheckSignatureFrom(resp *ocsp.Response, issuer *x509.Certificate) error {
	return  gmx509.GetX509SM2().CheckCertSignature(issuer,resp.SignatureAlgorithm, resp.TBSResponseData, resp.Signature)
}
